...
function clearSet(set) {
  ...
  if (set.size > 0) {
    // Використання методу clear для очищення множини, якщо розмір більше нуля
    set.clear();
...
function addElements(mySet, arr) {
  ...
  for (let element of arr) {
    // Додавання кожного елемента до множини за допомогою методу add
    mySet.add(element);
  }
  return mySet;
}
...
function setToArray(mySet) {
  ...
  const array = [...mySet]; // Конвертуємо множину в масив за допомогою деструктуризації
  const stringElements = array.filter((element) => typeof element === "string"); // Фільтруємо лише рядкові елементи
  const sortedArray = stringElements.sort(); // Сортуємо рядкові елементи в алфавітному порядку
  return sortedArray; // Повертаємо відсортований масив
}
...
function removeDuplicatesInPlace(arr) {
  ...
  const uniqueSet = new Set(); // Створюємо множину для збереження унікальних елементів

  for (let i = 0; i < arr.length; i++) {
    if (uniqueSet.has(arr[i])) {
      arr.splice(i, 1); // Видаляємо дублікат з масиву
